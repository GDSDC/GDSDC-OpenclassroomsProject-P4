...
==============================
    """
        ...
        choix_utilisateur_menu = pvt.parse_and_validate(
            ...
            parse=pvt.parse_int,
            validate=lambda x: pvt.validate_integer_interval(parsed_int=x, interval=(0, len(menu) - 1)),
        )
...
"""
        ...
        nouveau_tournoi["nom"] = pvt.parse_and_validate(
            explanation=nouveau_tournoi_texte_nom, parse=pvt.parse_string_not_empty, validate=pvt.no_validation
        )

        # Tournament venue definition
        nouveau_tournoi_texte_lieu = "\nRenseignez le Lieu du tournoi : "
        nouveau_tournoi["lieu"] = pvt.parse_and_validate(
            explanation=nouveau_tournoi_texte_lieu, parse=pvt.parse_string_not_empty, validate=pvt.no_validation
        )
        ...
        nouveau_tournoi["controle_du_temps"] = pvt.parse_and_validate(
            explanation=nouveau_tournoi_texte_controle_du_temps,
        ...
        nouveau_tournoi["description"] = pvt.parse_and_validate(
            explanation=nouveau_tournoi_texte_description, parse=pvt.parse_string_not_empty, validate=pvt.no_validation
        )
    ...
    def selectionner_tournoi(self, tournois: List[Tournoi]) -> Tournoi:
        ...
        tournoi_selectionne = pvt.parse_and_validate(
            explanation=selectionner_tournoi_texte,
            parse=pvt.parse_int,
            validate=lambda x: pvt.validate_integer_interval(parsed_int=x, interval=(1, len(tournois))),
        )
        # Return
...
"""
        ...
        if acteurs:
            ...
            choix_acteurs_texte = "\nChoisissez un acteur parmis la liste en entrant son numéro " \
                                  "ou entrez 'terminer' lorsque vous avez terminé la selection : "
            exit_condition = False
            while not exit_condition:
                acteur_key = pvt.parse_and_validate(
                    explanation=choix_acteurs_texte, parse=pvt.parse_string_not_empty, validate=pvt.validate_actor_key
                )
                if acteur_key == "terminer":
                    ...
                    if acteurs[acteur_key] in choix_acteurs:
                        print(
                            f"Joueur {acteurs[acteur_key].prenom} {acteurs[acteur_key].nom_de_famille} "
                            f"déjà sélectionné XX"
                        )
...
==========================================
                ...
                nouveaux_joueurs[joueur - 1]["date_de_naissance"] = pvt.parse_and_validate(
                    explanation=nouveau_joueur_texte_date_naissance,
                    parse=pvt.parse_date,
                    validate=pvt.validate_date_format,
                )

                # Definition of sex
                nouveau_joueur_texte_sexe = f'\nRenseignez le sexe du joueur n°{joueur} ("m"/"f") :'
                nouveaux_joueurs[joueur - 1]["sexe"] = pvt.parse_and_validate(
                    explanation=nouveau_joueur_texte_sexe, parse=pvt.parse_string_not_empty, validate=pvt.validate_sexe
                )
                ...
                nouveaux_joueurs[joueur - 1]["classement"] = pvt.parse_and_validate(
                    explanation=nouveau_joueur_texte_classement,
                    parse=pvt.parse_int,
                ...
                # Message
                print(
                    f"\nNouveau Joueur {nouveaux_joueurs[joueur - 1].prenom} "
                    f"{nouveaux_joueurs[joueur - 1].nom_de_famille} ajouté avec succès !"
                )
...
==============================
            ...
            choix_acteurs_texte = "\nChoisissez un acteur parmis la liste en entrant son numéro : "
            acteur_key = pvt.parse_and_validate(
                explanation=choix_acteurs_texte,
                parse=pvt.parse_int,
                validate=lambda x: pvt.validate_integer_interval(parsed_int=x, interval=(1, len(acteurs_liste))),
            )
            joueur_a_supprimer = acteurs_liste[acteur_key - 1]
            # Message
            print(
                f"\nJoueur {joueur_a_supprimer.prenom} {joueur_a_supprimer.nom_de_famille} "
                f"supprimé de la liste des acteurs avec succès !"
            )
...
"""
        ...
        for ((joueur1, score1), (joueur2, score2)) in round_param.match_liste:
            print(
                f"Paire n°{paires} : {joueur1.prenom} {joueur1.nom_de_famille} / "
                f"{joueur2.prenom} {joueur2.nom_de_famille}"
            )
            paires += 1
    ...
    def creer_nouveau_round(self, numero_round: int) -> Round:
        ...
        # Initialization
        nouveau_round = {"nom": "", "match_liste": [], "date_debut": "", "date_fin": ""}
...
"""
        ...
        # Loop on matches
        for idx, ((joueur1, score_joueur1), (joueur2, score_joueur2)) in enumerate(match_liste_scores):
            ...
            resultat_match = pvt.parse_and_validate(
                explanation=match_texte,
                parse=pvt.parse_int,
                validate=lambda x: pvt.validate_integer_interval(parsed_int=x, interval=(0, 2)),
            )
            # Scoring
            if resultat_match == 1:
...
"""
        ...
        for joueur in joueurs_ancien_classement:
            joueur_classement_texte = f"{joueur.prenom} {joueur.nom_de_famille} / " \
                                      f"Classement actuel = {joueur.classement} --> Nouveau classement : "
            nouveau_classement = pvt.parse_and_validate(
                explanation=joueur_classement_texte, parse=pvt.parse_int, validate=pvt.validate_integer_positive
            )
            joueur_classement_a_ajour = Joueur(
...
"""
    ...
    def selectionner_acteur(self, acteurs_liste: List[Joueur]) -> Joueur:
        ...
        indice_acteur_selectionne = pvt.parse_and_validate(
            explanation=selectionner_acteur_texte,
            parse=pvt.parse_int,
            validate=lambda x: pvt.validate_integer_interval(parsed_int=x, interval=(1, len(acteurs_liste))),
        )
    ...
    def afficher_rapport_joueurs_alphabetique(
        ...
        self.afficher_rapports(
            nom_rapport=f"joueurs du tournoi {tournoi.nom} par ordre alphabétique", donnees_rapport=formatted_data
        )
    ...
    def afficher_rapport_joueurs_classement(self, tournoi: Tournoi, donnees_rapport: List[Joueur]):
        ...
        formatted_data = self.format_players_data(data=donnees_rapport)
        self.afficher_rapports(
            nom_rapport=f"joueurs du tournoi {tournoi.nom} par ordre de classement", donnees_rapport=formatted_data
        )
    ...
    def afficher_rapport_tournois(self, donnees_rapport: List[Tournoi]):
        ...
        formatted_data = [
            f"Tournoi {tournoi.nom} de {tournoi.lieu} qui a débuté le : {tournoi.date_debut.strftime('%d-%m-%Y')}"
            for tournoi in donnees_rapport
        ]
    ...
    def afficher_rapport_tours_tournoi(self, tournoi: Tournoi, donnees_rapport: List[Round]):
        ...
        if not donnees_rapport:
            ...
            formatted_data = [
                ...
                for round_variable in donnees_rapport
            ]
    ...
    def afficher_rapport_matchs_tournoi(self, tournoi: Tournoi, donnees_rapport: List[Match]):
        ...
        if not donnees_rapport:
            ...
            formatted_data = [
                ...
                for ((joueur_1, _), (joueur_2, _)) in donnees_rapport
            ]
